...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (orderForm) {
    ...
    orderForm.addEventListener('submit', (e) => {
      ...
      // ปรับสถานะปุ่มเป็นกำลังส่งข้อมูล
  ...
  if (ordersTableBody) {
    // ฟังก์ชันช่วยแยกคอลัมน์จากข้อมูล CSV
    ...
    const fetchOrdersFromCSV = () => {
      ...
      fetch(csvURL)
        ...
        .then(csvText => {
          ...
          // หากมีเฉพาะแถว Header หรือไม่มีข้อมูล
          if (lines.length <= 1) {
            ordersTableBody.innerHTML = `
              <tr>
          ...
          // เรียงรายการล่าสุดขึ้นก่อน (Reverse)
          ...
          ordersTableBody.innerHTML = `
            <tr>
              <td colspan="5" class="py-8 text-center text-rose-500">
                ไม่สามารถโหลดข้อมูลคำสั่งซื้อได้ กรุณาตรวจสอบว่า Sheet ได้ทำการ Publish to web แบบ CSV แล้วหรือไม่
              </td>
            </tr>
          `;
        });
    ...
    // โหลดข้อมูลทันทีเมื่อเปิดหน้า admin.html
    fetchOrdersFromCSV();
